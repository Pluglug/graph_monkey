...
class MonKeyKeymapSettings(PropertyGroup):
    ...
    view_focus_key: StringProperty(
        name="Focus", description="Key for focus selected curves", default="F", maxlen=1
    )
    ...
    def update_keymaps(self, context):
        ...
        config.set_key_for_action(
            "vertical_movement", "upward", self.vertical_upward_key
        )
        config.set_key_for_action(
            "vertical_movement", "downward", self.vertical_downward_key
        )
        config.set_key_for_action(
            "horizontal_movement", "forward", self.horizontal_forward_key
        )
        config.set_key_for_action(
            "horizontal_movement", "backward", self.horizontal_backward_key
        )
    ...
    def execute(self, context):
        ...
        if conflicts:
            conflict_msg = "Keymap conflicts found:\n" + "\n".join(
                [f"• {c1} vs {c2}" for c1, c2 in conflicts]
            )
