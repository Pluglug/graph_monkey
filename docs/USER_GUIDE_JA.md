...
すべて **Alt+キー** の組み合わせで操作します。Shift を追加すると拡張選択になります。
...
再生範囲内でキーフレーム間をジャンプします。**キーフレームタイプによるフィルタリング**に対応。
...
**キーフレームタイプフィルター**:
Timeline/Dopesheetヘッダーに表示されるフィルターで、以下のタイプを選択的にジャンプ対象にできます：
...
![Action管理ツールバー](images/action_toolbar.png)
...
![アドオン設定画面](images/preferences.png)
