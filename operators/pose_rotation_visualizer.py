...
class PoseRotationVisualizerSettings(bpy.types.PropertyGroup):
    ...
    display_style: EnumProperty(
        ...
        items=[
            ...
            ("DIFF_ONLY", "Difference Only", "Display only rotation difference indicators"),
        ],
        default="BOTH",
    )
...
class PoseRotationVisualizerHandler:
    ...
    def _draw_bone_rotation(self, context, pose_bone, settings):
        ...
        current_origin, current_x, current_y, current_z = get_bone_axes_world(pose_bone, rest_pose=False)
        
        # 回転差分を取得
        _, angle_degrees = get_rotation_difference(pose_bone)
    ...
    def _draw_axes(self, origin, x_axis, y_axis, z_axis, length, alpha=1.0, use_rest_colors=False):
        """Draw XYZ axes."""
        if use_rest_colors:
            # レストポーズは半透明グレー
            colors = [
